...
export default function Settings() {
		...
		const handlePasswordChange = async () => {
			...
			try {
				...
				console.error("Error updating passwords");
			}
		};
		...
		const handleUsernameChange = async () => {
			try {
				axios.post(apiUrl + "/changeusername", {username}, {
						headers: {
								'Content-Type': 'application/x-www-form-urlencoded',
								Authorization: `Bearer ${token}`, 
						},
					...
					<div className="space-y-2">
						...
						<div className="flex space-x-2">
							...
							<button 
								...
								className="px-4 py-2 bg-gray-800 text-white border border-gray-700 rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
							>
								Change
							</button>
						</div>
